...
Figures and media live under `assets/`, named with the chapter that owns them:

| Path | What goes there |
|---|---|
| `assets/figs/chNN-slug.png` | Static figures — diagrams, infographics, anything not generated by an R chunk |
| `assets/video/chNN-slug.{mp4,png}` | Manim clips and their poster stills, built by `manim/build.py` |
| `assets/tables/` | Table images, if a table ever has to ship as an image |

Plots produced by an R chunk are **not** stored here — they are regenerated on every render and
live in `_freeze/`, which is gitignored.

**Every static figure carries `fig-alt`.** It is what a screen reader announces, and Quarto turns
it into `\includegraphics[alt={...}]` in the PDF, which is why `styles/pdf-preamble.tex` defines
that graphics key as a no-op — without it the PDF build fails outright with
